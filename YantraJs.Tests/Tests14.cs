...
public class Tests14 : Base
{
    ...
    public void Type_With_Native_Resource()
    {
        ...
        try
        {
            ...
            writer.Write("1");
            StreamWriter cloned = writer.YantraClone();
            writer.Write("2");
            cloned.Write("3");
    ...
    [Test]
    public void CaseFuncs()
    {
        string[] closure = ["123"];
        Func<int, string> f = x => closure[0] + x.ToString(CultureInfo.InvariantCulture);
        Func<int, string> df = f.YantraClone();
        Func<int, string> cf = f.YantraShallowClone();
        closure[0] = "xxx";
    ...
    public void Events()
    {
        ...
        EventHandlerTest1 clone = eht.YantraClone();
        clone.Call(1);
        // do not call
        Assert.That(summ[0], Is.EqualTo(2));
        eht.Event -= a1;
